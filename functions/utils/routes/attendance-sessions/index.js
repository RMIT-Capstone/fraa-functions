module.exports = {
  CREATE_ATTENDANCE_SESSION: 'create_attendance_session',
  GET_ATTENDANCE_SESSIONS_IN_DATE_RANGE: 'get_attendance_sessions_in_date_range',
  GET_ATTENDANCE_SESSIONS_IN_MONTH_RANGE: 'get_monthly_attendance_sessions_in_month_range',
  GET_MORE_ATTENDANCE_SESSIONS_IN_DATE_RANGE: 'get_more_attendance_sessions_in_date_range',
  GET_DAILY_ATTENDANCE_SESSION: 'get_daily_attendance_sessions',
  GET_MONTHLY_ATTENDANCE_SESSIONS: 'get_monthly_attendance_sessions',
};
